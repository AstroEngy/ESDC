% Mass Budget Propulsion System
%
% Derives the propulsion system mass estimate based on individual components
% with scaling applied to each component (tank, thruster, and PPU if available)
%
% Inputs:
%   data            - Structure containing propulsion system parameters
%   mass_propellant - Total propellant mass [kg]
%
% Outputs:
%   mass_epropulsion_power - Structure containing individual component masses
%                            and total propulsion system mass
...
function [mass_epropulsion_power] = mass_budget_propulsion(data, mass_propellant)
    ...
    % Tank mass scaling
    % TODO: Rewrite function with correlates, add margin to tank sizing
    mass_epropulsion_power.tank = m_scale_tank(mass_propellant, data.propellant);
    
    % Thruster mass estimation by correlation
    mass_epropulsion_power.thruster = m_scale_thruster(...
        data.power_thruster, ...
        data.propulsion_system, ...
        data.propellant);
    
    % PPU (Power Processing Unit) mass - only if scaling data exists
    ppu_scaling_file = sprintf(...
        "Database/Scaling/scaling_propulsion_system_%s_ppu_mass_to_power.csv", ...
        data.propulsion_system);
    
    if exist(ppu_scaling_file, 'file')
        mass_epropulsion_power.PPU = m_scale_PPU(...
            data.power_thruster, ...
            data.propulsion_system);
    end
    
    % TODO: Consider propulsion structure mass estimate based on % margin
    %       (piping, valves, transducers, etc.)
    % mass_epropulsion_power.structure = m_scale_structure(data.propulsion_system);
    
    % ===== Total Mass Calculation =====
    
    % Sum all component masses
    component_fields = fieldnames(mass_epropulsion_power);
    mass_total = 0;
    
    for i = 1:numel(component_fields)
        mass_total = mass_total + mass_epropulsion_power.(component_fields{i});
    end
